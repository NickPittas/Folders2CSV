...
# --------------------
# Naming Requirements (for reference)
# --------------------
requirements_str = (
    "Naming requirements:\n"
    ...
    "- Then an underscore and a mandatory pixel mapping (LL180 or LL360) immediately followed by resolution (digits followed by 'k' or 'K').\n"
    "- Then an underscore and a colorspace+gamma field (no underscores allowed).\n"
    "- Then an optional underscore and fps (digits) may appear (but is mandatory for sequences and video files).\n"
    ...
    "- For video files, only a dot and extension follow.\n"
)

# --------------------
# Validate a file name against criteria.
# Returns (errors, warnings) lists (only specific issues).
# --------------------
def validate_filename(basename):
    ...
    if not re.match(r'^[A-Za-z]{3,4}\d{4}$', token0):
        errors.append("Token0 must be 3-4 letters followed by 4 digits")
    ...
    # Since pixelMapping is now mandatory, no warning needed.
    return errors, warnings

# --------------------
# Updated Regex: PixelMapping is mandatory; frame_padding accepts any number of digits.
# --------------------
...
class FileScanner(QThread):
    ...
    def build_data_dict(self, f, m):
        groups = m.groupdict()
        basename = os.path.basename(f)
        # For image sequences: if frame_padding exists, compute common base.
        if groups.get("frame_padding"):
            common_base = re.sub(r'([_.]\d+)\.[^.]+$', '', basename)
            version_name = common_base
            # Count how many files in the same directory match the sequence.
            directory = os.path.dirname(f)
            listing = self.get_dir_listing(directory)
            # Build a pattern that matches filenames starting with the common_base followed by a separator and digits, and then a dot and the same extension.
    ...
    def run(self):
        ...
        sequences = {}  # Group by (Version Name, file type) to include only the first file of a sequence.
