...
# --------------------
# This regex is designed to capture the following groups:
# - sequence: exactly 4 letters
# - shotNumber: exactly 4 digits
# - description: text until the next underscore
# - Optional pixelMapping (LL180 or LL360)
# - resolution: digits and letters (e.g. 2k, 1k, 12k)
# - colorspaceGamma: a field we ignore in CSV
# - fps: digits (frame rate, e.g., 2997 for 29.97 fps)
# - version: digits following "v" (will be reformatted to 3 digits with a leading "v")
# - For image files, an optional frame_padding (if present) and extension;
#   for video files, only the extension.
new_pattern = re.compile(
    r'^(?P<sequence>[A-Za-z]{4})(?P<shotNumber>\d{4})_'                # sequence and shotNumber
    ...
    r'(?P<colorspaceGamma>[^_]+)_'                                      # colorspace+gamma field (ignored)
    r'(?P<fps>\d+)_'
...
class FileScanner(QThread):
    ...
    def run(self):
        # Gather all candidate files from the selected directories.
        candidate_files = []
        for directory in self.directories:
            for ft in self.file_types:
                candidate_files.extend(glob.glob(os.path.join(directory, '**', ft), recursive=True))
        # Filter files that match the new naming convention.
        ...
        sequences = {}  # to avoid duplicates based on Name
        ...
        for f in candidate_files:
            ...
            if not m:
                continue
            groups = m.groupdict()
            # Column 1: Name — for image files with frame_padding remove it.
            ...
            # Column 3: Version Number: reformat to 3-digit padded number with leading "v"
            try:
                version_number = "v" + format(int(groups['version']), "03d")
            except Exception:
                version_number = "v" + groups['version']
            # Column 4: Submitted For: empty by default.
            submitted_for = ""
            # Column 5: Notes: empty by default.
            notes = ""
            # Column 6: File Type: extension in upper-case.
            file_type_field = groups['extension'].upper()
            # Column 7: Resolution: the parsed resolution.
            resolution_field = groups['resolution']
            # Column 8: Duration: if image then "Still Frame"; if video, use OpenCV.
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        # Set window width to 40% smaller than 1625 (~975) and height 800.
