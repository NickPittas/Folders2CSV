...
# --------------------
# Expected Naming Requirements Description (for reference only)
# --------------------
requirements_str = (
    "Naming requirements:\n"
    "- File name must start with exactly 3-4 letters followed by exactly 4 digits.\n"
    "- Followed by an underscore and a description (letters, digits, or hyphens).\n"
    ...
    "- Then an underscore and a colorspace+gamma field (with no underscores).\n"
    "- Then an optional underscore and fps (digits) may appear. For image sequences (files with frame padding) and video files, fps is mandatory; for single images, fps is optional.\n"
    "- Then an underscore, then 'v' followed by 1-4 digits (the version).\n"
    "- For image files, if frame padding is present (indicating a sequence), it must be preceded by an underscore or dot before the final dot and extension.\n"
    "- For video files, a dot and extension follow.\n"
)

# --------------------
# Validate a file name against expected criteria.
# Returns (errors, warnings) lists containing only the specific issues.
# --------------------
def validate_filename(basename):
    ...
    if not m:
        ...
        return errors, warnings
    groups = m.groupdict()
    # Check sequence+shotNumber: must be exactly 3-4 letters followed by 4 digits.
...
class FileScanner(QThread):
    ...
    def __init__(self, directories, delivery_package, file_types):
        ...
        self.file_types = file_types  # e.g. ["*.mp4", "*.exr", ...]
        self.data = []
        self.paused = False
    ...
    def process_file(self, f):
        ...
        # Create a unique key for grouping by sequence.
        key = None
        m = new_pattern.match(basename)
        if m:
            groups = m.groupdict()
    ...
    def build_data_dict(self, f, m):
        groups = m.groupdict()
        if groups.get("frame_padding"):
            version_name = os.path.basename(f).rsplit('.', 2)[0]
        else:
            version_name = os.path.splitext(os.path.basename(f))[0]
        data_key = (version_name, groups['extension'].lower())
        shot_name_field = f"{groups['sequence']}_{groups['shotNumber']}" if groups['shotNumber'] else groups['sequence']
        try:
    ...
    def run(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if not result['valid']:
                    ...
                    continue
                # For valid files, log warnings only once per key.
        ...
        sequences = {}  # Key = (version_name, extension)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Log window (only shows errors/warnings)
