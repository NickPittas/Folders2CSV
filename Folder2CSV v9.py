...
requirements_str = (
    "Naming requirements:\n"
    "- The file name must start with exactly 3-4 letters (the sequence).\n"
    "- Optionally followed by a four-digit shot number (if present, must be 4 digits) – its underscore is also optional.\n"
    "- Then an underscore and a description (letters, digits, or hyphens) (mandatory).\n"
...
def validate_filename(basename):
    ...
    if not m:
        ...
        return errors, warnings
    groups = m.groupdict()
    # Build token0 as sequence + shotNumber if provided.
    token0 = groups['sequence'] + (groups['shotNumber'] or '')
    if groups.get('shotNumber') and not re.match(r'^\d{4}$', groups['shotNumber']):
        errors.append("Shot number, if provided, must be exactly 4 digits")
    ...
    # Try to get the extension from either branch.
    ext = (groups.get('extension') or groups.get('extension2') or '').lower()
...
# --------------------
# Updated regex:
# - The shot number (four digits followed by an underscore) is optional.
# - The pixelMapping group is now optional.
# - Frame padding now uses \d+ to accept any number of digits.
# - The tail has two alternatives: one for sequence files (with frame_padding) and one for single files.
# --------------------
new_pattern = re.compile(
    ...
    r'(?:_(?P<fps>\d+))?_' +
    r'v(?P<version>\d{1,4})' +
    r'(?:(?:[_\.](?P<frame_padding>\d+))\.(?P<extension>[^.]+)|\.(?P<extension2>[^.]+))$',
    re.IGNORECASE
)
...
class FileScanner(QThread):
    ...
    def process_file(self, f):
        ...
        if m:
            groups = m.groupdict()
            key = (groups['sequence'], groups.get('shotNumber') or '', groups['description'],
    ...
    def build_data_dict(self, f, m):
        groups = m.groupdict()
        basename = os.path.basename(f)
        # For image sequences: if frame_padding is provided, treat as a sequence.
        if groups.get("frame_padding"):
            # For the Version Name column, we want the full filename without extension (thus including frame padding).
            version_name = os.path.splitext(basename)[0]
            # For grouping, we compute a common base by stripping off the trailing separator and digits.
            ...
            # For single files, simply remove the extension.
            version_name = os.path.splitext(basename)[0]
            duration_field = "Still Frame"
            grouping_key = version_name
        # Build Shot Name using sequence; if shotNumber is present append it.
        shot_name_field = groups['sequence']
        if groups.get('shotNumber'):
            shot_name_field += "_" + groups['shotNumber']
        try:
        ...
        submitted_for = ""
        delivery_notes = ""
        # For file type, use the extension from the first alternative, or extension2.
        ext = groups.get('extension') or groups.get('extension2') or ''
        file_type_field = ext.upper()
    ...
    def run(self):
        ...
        self.log_message.emit("Found " + str(total_candidates) + " candidate files.")
        valid_files = []
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if not result['valid']:
                    if result['key'] not in logged_error_keys:
                        self.log_message.emit("File '" + result['basename'] + "' rejected: " + "; ".join(result['errors']))
                        logged_error_keys.add(result['key'])
                    continue
                if result.get('warnings'):
                    if result['key'] not in logged_warning_keys:
                        self.log_message.emit("File '" + result['basename'] + "' warning: " + "; ".join(result['warnings']))
                        logged_warning_keys.add(result['key'])
                valid_files.append(result['data_dict'])
        self.log_message.emit(str(len(valid_files)) + " files match the naming convention out of " + str(total_candidates) + " candidates.")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.start_scan_button.setMinimumSize(50, 40)
        self.start_scan_button.clicked.connect(self.start_scan)
        self.save_csv_button = QPushButton("Save CSV")
        self.save_csv_button.setMinimumSize(50, 40)
